...
public class PnQuanLyBanHangGUI extends JPanel {
    ...
    private void addControlsBanHang() {
        ...
        JPanel pnMaKM = new TransparentPanel(new FlowLayout());
        pnMaKM.add(lblMaKM);
        lblMaKM.setFont(font);
        txtMaKM.setFont(font);
        pnMaKM.add(txtMaKM);
        pnCTHoaDonLeft.add(pnMaKM);
    ...
    private void loadDataComboboxNhanVienBan() {
        ...
        if (dsnv != null) {
            for (NhanVien nv : dsnv) {
                cmbNhanVienBan.addItem(nv.getMaNV() + " - " + nv.getTen() + " " + nv.getNgaySinh());
            }
        }
    ...
    private void xuLyClickTblBanHang() {
        ...
        if (row > -1) {
            ...
            if (soLuong < 1) {
                Dialog dlg = new Dialog("Sản phẩm đã hết hàng", Dialog.ERROR_DIALOG);
                return;
            }
    ...
    private void xuLyThemVaoGioHang() {
        ...
        if (soLuong > soLuongConLai || soLuongConLai <= 0) {
            new Dialog("Sản phẩm đã hết hàng", Dialog.ERROR_DIALOG);
            return;
        }
        ...
        for (int i = 0; i < tblGioHang.getRowCount(); i++) {
            ...
            if (maTbl == key) {
                ...
                dtmGioHang.setValueAt(soLuongAdd, i, 2);
                dtmGioHang.setValueAt(dcf.format(soLuongAdd * donGiaSP), i, 4);
    ...
    private void xuLyHienCTHoaDon() {
        ...
        try {
            Date d = sdf.parse(hd.getNgayLap());
            sdf = new SimpleDateFormat("dd/MM/yyyy");
            String ngayLap = sdf.format(d);
